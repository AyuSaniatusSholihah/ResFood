const fs = require('fs');
const appFile = 'd:/Nia/lomba/ResFood/frontend/src/App.jsx';
const catalogFile = 'd:/Nia/lomba/ResFood/frontend/src/pages/CatalogPage.jsx';
const editFile = 'd:/Nia/lomba/ResFood/frontend/src/pages/EditProductPage.jsx';
const backendIndex = 'd:/Nia/lomba/ResFood/backend/index.js';

// 1. Update App.jsx to accept ID param
let appContent = fs.readFileSync(appFile, 'utf8');
appContent = appContent.replace('<Route path="/edit-produk" element={<EditProductPage />} />', '<Route path="/edit-produk/:id" element={<EditProductPage />} />');
fs.writeFileSync(appFile, appContent, 'utf8');

// 2. Update CatalogPage.jsx to pass ID
let catalogContent = fs.readFileSync(catalogFile, 'utf8');
catalogContent = catalogContent.replace(/to="\/edit-produk"/g, 'to={`/edit-produk/${item.id}`}');
fs.writeFileSync(catalogFile, catalogContent, 'utf8');

// 3. Update backend/index.js to support GET and PUT for specific item
let backendContent = fs.readFileSync(backendIndex, 'utf8');
if (!backendContent.includes('app.get(\'/api/makanan/:id\'')) {
  const newRoutes = `
...
app.get('/api/makanan/:id', async (req, res) => {
  try {
    const makanan = await prisma.makanan.findUnique({
      ...
      include: { penyedia: { select: { nama: true } } }
    });
...
`;
  backendContent = backendContent.replace('app.listen(port', newRoutes + '\napp.listen(port');
  fs.writeFileSync(backendIndex, backendContent, 'utf8');
}

// 4. Complete rewrite of EditProductPage.jsx to fetch and bind data
let editContent = fs.readFileSync(editFile, 'utf8');
const newEditCode = `import React, { useState, useEffect } from 'react';
...
fs.writeFileSync(editFile, newEditCode, 'utf8');

console.log('EditProduct feature implemented successfully.');
